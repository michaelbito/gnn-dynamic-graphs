...
class FAConvolution(tf.keras.layers.Layer): 
    def __init__(self, units, heads, activation=tf.nn.relu, dropout=.2, bias=0, final_layer=False):
        ...
        self.EPSILON = .1 # hardcoded strength of initial residual connection
        self.INIT_DIM = 3 # hardcoded initial dimension
        
    def build(self, input_shape): 
        self.w_init = self.add_weight(shape=(self.INIT_DIM, self.units), initializer='glorot_normal', trainable=True)
    ...
    def call(self, x, x_initial, adj):
        ...
        attns = attns + self.EPSILON * tf.linalg.matmul(x_initial, self.w_init)
...
class FAGCN(tf.keras.Model): 
    def __init__(self, num_layers, n_classes, bias, dropout, optimizer, loss): 
        ...
        for layers in range(self.num_layers - 1):
            self.model_layers.append(FAConvolution(units=32, heads=1, dropout=self.dropout, bias=self.bias))
        self.model_layers.append(FAConvolution(units=1, heads=1, dropout=self.dropout, bias=self.bias, activation=tf.nn.sigmoid, final_layer=True))
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            y_hat = self(inputs=X, adj=adj_mean, training=True)
    ...
    def test_step(self, data):
        X, y, y_mask, adj, adj_mean, adj_norm = data
        y_mask = tf.cast(y_mask, tf.bool)
        y_hat = self(inputs=X, adj=adj_mean, training=False)
        y_hat = tf.boolean_mask(y_hat, y_mask)
        y = tf.boolean_mask(y, y_mask)
