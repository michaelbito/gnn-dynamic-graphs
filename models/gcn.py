...
class GraphConvolution(tf.keras.layers.Layer):
    def __init__(self, units=32, activation=tf.nn.relu, dropout=.2, g_dropout=.5, bias=False): 
...
class GCN(tf.keras.Model): 
    def __init__(self, 
        ...
        for layers in range(self.num_layers - 1):
            self.model_layers.append(GraphConvolution(units=32, bias=bias, dropout=dropout, g_dropout=g_dropout))
        self.model_layers.append(GraphConvolution(units=1, bias=bias, dropout=dropout,g_dropout=g_dropout, activation=tf.nn.sigmoid))
