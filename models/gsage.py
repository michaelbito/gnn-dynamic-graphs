...
class SageConvolution(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.w_ego = self.add_weight(shape=(input_shape[-1], self.units), initializer='glorot_normal', trainable=True)
        self.w_neighbor = self.add_weight(shape=(input_shape[-1], self.units), initializer='glorot_normal', trainable=True)
        if self.bias: 
    ...
    def call(self, inputs, adj): 
        ...
        x_ego = tf.linalg.matmul(x, self.w_ego)
        x_neighbor = tf.linalg.matmul(x, self.w_neighbor)
        x = tf.sparse.sparse_dense_matmul(adj, x_neighbor) + x_ego
...
class GSAGE(tf.keras.Model): 
    def __init__(self, 
        ...
        for layers in range(self.num_layers - 1):
            self.model_layers.append(SageConvolution(units=32, bias=bias, dropout=dropout))
        self.model_layers.append(SageConvolution(units=1, bias=bias, dropout=dropout, activation=tf.nn.sigmoid))
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            y_hat = self(inputs=X, adj=tf.sparse.from_dense(adj_mean), training=True)
    ...
    def test_step(self, data):
        X, y, y_mask, adj_sum, adj_mean, adj_sym = data
        y_mask = tf.cast(y_mask, tf.bool)
        y_hat = self(inputs=X, adj=tf.sparse.from_dense(adj_mean), training=False)
        y_hat = tf.boolean_mask(y_hat, y_mask)
        y = tf.boolean_mask(y, y_mask)
